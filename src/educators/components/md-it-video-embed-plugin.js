...
function videoEmbed(md, options) {
  function videoReturn(state, silent) {
    ...
    if (!silent) {
      ...
      token = theState.push('embededVideo', '');
...
export default function videoPlugin(md, options) {
  ...
  theMd.renderer.rules.embededVideo = tokenizeVideo(theMd, theOptions);
  theMd.inline.ruler.before('emphasis', 'embededVideo', videoEmbed(theMd, theOptions));
}
